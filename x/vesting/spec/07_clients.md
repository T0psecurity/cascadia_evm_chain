...
Find below a list of `cascadiad` commands added with the `x/vesting` module. You can obtain the full list by using the `cascadiad -h` command.
...
```go
cascadiad add-genesis-account [address_or_key_name] [coin][,[coin]] [flags]
```
...
```go
cascadiad query vesting balances [address] [flags]
```
...
```go
cascadiad tx vesting create-clawback-vesting-account [to_address] [flags]
```
...
```go
cascadiad tx vesting clawback [address] [flags]
```
