...
Get started with your Ledger hardware wallet on Cascadia {synopsis}
...
If you want to connect to Cascadia mainnet and Cascadia testnet, you can use the Ethereum Ledger app on Ledger Live by setting the chain ID.
...
|               | EIP155 chain ID |
| ------------- | --------------- |
| Cascadia mainnet | `9001`          |
| Cascadia testnet | `9000`          |
...
::: tip
Follow our [Metamask Guide](./metamask.md) to add the Cascadia Mainnet and Testnet to your Settings
:::
...
- `msgs`: This is the content of the cosmos transaction, in this example, we need to make sure that we are using a MsgSend, and that the *to_address* is the one that we want to send the founds. Also, we can verify that we are actually sending *10000uCC* to that wallet.
...
- Make sure that the Ethereum Ledger app is installed. The Cosmos Ledger app is not supported on the Cascadia chain at the moment (see [FAQ](#faq)).
...
- If you have Metamask correctly configured, the balance on the extension will be displayed as `CC`, but on the Ledger device it will be displayed as `ETH`.
...
Cosmos `secp256k1` keys are not supported on Cascadia with Ledger. Only Ethereum keys (`eth_secp256k1`) can be generated with Ledger.

2. **I can’t generate keys using the CLI with `cascadiad` with the `--ledger` flag**

CLI bindings with `cascadiad` binary are not currently supported. In the meantime, you can use the Ethereum Ledger App with EIP712 using [evmos.me](https://evmos.me). See the [`EIP712 Signing`](#eip712-signing) section for reference.

3. **I can’t generate a key for the Cascadia native multisig using the `cascadiad` CLI and and Ledger**

You can generate a multisig wallet using the `cascadiad` CLI, although the `--ledger` option is not available at the moment.

4. **I can’t use Metamask or Keplr with the Cosmos Ledger app**

Since Cascadia only support Ethereum keys and uses the same HD path as Ethereum, the Cosmos Ledger app doesn’t work to sign cosmos transactions.
