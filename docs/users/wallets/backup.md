...
```bash
$ cascadiad keys add mykey-restored --recover
...
```bash
cascadiad keys unsafe-export-eth-key mykey > mykey.export
...
```bash
$ cascadiad keys import mykey-imported ./mykey.export
Enter passphrase to decrypt your key:
```
...
```
$ cascadiad keys unsafe-import-eth-key mykey-imported ./mykey.export
