...
echo "export DAEMON_NAME=cascadiad" >> ~/.profile
echo "export DAEMON_HOME=$HOME/.cascadiad" >> ~/.profile
source ~/.profile
```

After this, you must make the necessary folders for `cosmosvisor` in your `DAEMON_HOME` directory (`~/.cascadiad`) and copy over the current binary.

```bash
mkdir -p ~/.cascadiad/cosmovisor
mkdir -p ~/.cascadiad/cosmovisor/genesis
mkdir -p ~/.cascadiad/cosmovisor/genesis/bin
mkdir -p ~/.cascadiad/cosmovisor/upgrades

cp $GOPATH/bin/cascadiad ~/.cascadiad/cosmovisor/genesis/bin
```

To check that you did this correctly, ensure your versions of `cosmovisor` and `cascadiad` are the same:
...
sudo -S systemctl enable cascadiad
sudo -S systemctl start cascadiad
```
