...
`cascadiad` is the all-in-one command-line interface. It supports wallet management, queries and transaction operations {synopsis}
...
After you have obtained the latest `cascadiad` binary, run:
...
::: tip
You can also enable auto-completion with the `cascadiad completion` command. For example, at the start of a bash session, run `. <(cascadiad completion)`, and all `cascadiad` subcommands will be auto-completed.
:::

### Config and data directory

By default, your config and data are stored in the folder located at the `~/.cascadiad` directory.
...
To specify the `cascadiad` config and data storage directory; you can update it using the global flag `--home <directory>`

### Configuring the Node

The Cosmos SDK automatically generates two configuration files inside `~/.cascadiad/config`:
...
We can view the default client config setting by using `cascadiad config` command:
...
A list of commonly used flags of `cascadiad` is listed below:

| Option              | Description                   | Type                                             | Default Value |
| ------------------- | ----------------------------- | ------------------------------------------------ | ------------- |
| `--chain-id`        | Full Chain ID                 | `string`                                         | `""`          |
| `--home`            | Directory for config and data | `string`                                         | `~/.cascadiad`   |
| `--keyring-backend` | Select keyring's backend      | `{"os"|"file"|"kwallet"|"pass"|"test"|"memory"}` | `"os"`        |
| `--output`          | Output format                 | `string`                                         | "text"        |

## Command list

A list of commonly used `cascadiad` commands. You can obtain the full list by using the `cascadiad -h` command.
