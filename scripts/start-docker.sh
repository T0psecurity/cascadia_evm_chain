...
echo "init Cascadia with moniker=$MONIKER and chain-id=$CHAINID"
./cascadiad init $MONIKER --chain-id $CHAINID --home $DATA_DIR
echo "prepare genesis: Allocate genesis accounts"
./cascadiad add-genesis-account \
"$(./cascadiad keys show $KEY -a --home $DATA_DIR --keyring-backend test)" 1000000000000000000uCC,1000000000000000000stake \
